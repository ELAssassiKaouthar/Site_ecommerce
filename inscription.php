...
if(isset($_POST['login'])){
	if(!empty($_POST['username']) && !empty($_POST['username1']) && !empty($_POST['password']) && !empty($_POST['password_1']) && !empty($_POST['role']) && !empty($_POST['datenaissance'])  && !empty($_POST['email']))
    {
    ...
    exit();
			...
			$sql = "INSERT INTO utilisateur(email,nom,prenom,DateNaissance,mdp,role1) VALUES(:email,:nom,:prenom,:DateNaissance,:mdp,:role1)";
			$res = $pdo->prepare($sql);
			$exec = $res->execute(array(":email" => $email, ":nom" => $nom, ":prenom" => $prenom, ":mdp" => $password, ":DateNaissance" => $datedenaissance, ":role1" => $role));
			if($exec){
				echo "done"
